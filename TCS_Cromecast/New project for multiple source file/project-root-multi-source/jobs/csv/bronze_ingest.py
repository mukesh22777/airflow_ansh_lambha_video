...
def run():
    ...
    config["env"] = args.env
    spark = build_spark("csv-bronze-ingest", config)
    ...
    df = read_source(spark, config, "csv")
    validate_non_empty(df, "csv")
    df = add_load_date(df)

    watermark = get_watermark(config, "csv")
    ...
    set_watermark(config, "csv", datetime.now(timezone.utc).strftime("%Y-%m-%d"))
    logger.info("CSV bronze load completed to %s", bronze_path)
